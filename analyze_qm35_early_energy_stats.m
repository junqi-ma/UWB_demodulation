...
fprintf('occupancy:    min=%.2f  median=%.2f  max=%.2f  (debug)\n', ...
    min(stats.occupancy), median(stats.occupancy), max(stats.occupancy));
fprintf('cluster N:    min=%.0f  median=%.0f  max=%.0f   N>=5=%d\n', ...
    min(stats.cluster_n), median(stats.cluster_n), max(stats.cluster_n), ...
    nnz(stats.cluster_n >= 5));
...
tableOut = table(stats.packet_id, stats.schedule_index, stats.valid, ...
    ...
    stats.first_peak_delay_ns, stats.cluster_n, ...
    ...
    'state', 'first_path_delay_ns', 'first_peak_delay_ns', 'cluster_n', ...
    'early_tap_count', 'early_noise_power', 'early_residual_power', ...
    'signal_power', 'early_to_signal_db', 'r_early_db', 'r_peak_db', ...
...
summary = {
    ...
    sprintf('超过 R_early=-25 dB 的帧: %d  (debug)', nnz(stats.r_early_db > -25))
    sprintf('超过 occupancy=0.20 的帧: %d  (debug, 不决策)', nnz(stats.occupancy > 0.20))
    sprintf('簇 N>=5 (interfered / 默认 SIC): %d', nnz(stats.cluster_n >= 5))
    };
stateLine = "state:";
for i = 1:numel(u)
...
for k = 1:n
    ...
    if isfield(r, 'qm35_cluster_n')
        clusterN(k) = r.qm35_cluster_n;
    elseif isfield(d, 'cluster_n')
        clusterN(k) = d.cluster_n;
    end
    if isfield(r, 'qm35_first_peak_delay_ns')
        firstPeakDelay(k) = r.qm35_first_peak_delay_ns;
    elseif isfield(d, 'first_peak_delay_ns')
        firstPeakDelay(k) = d.first_peak_delay_ns;
    end
...
stats.cluster_n = clusterN;
stats.first_peak_delay_ns = firstPeakDelay;
