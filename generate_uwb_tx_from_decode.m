function tx = generate_uwb_tx_from_decode(decoded, options)
...
if ~isstruct(options) || ~isscalar(options)
    error('generate_uwb_tx_from_decode:InvalidOptions', ...
        'OPTIONS must be a scalar structure.');
end
unknown = setdiff(fieldnames(options), fieldnames(defaults));
if ~isempty(unknown)
    error('generate_uwb_tx_from_decode:UnknownOption', ...
...
if options.require_fcs_pass && ~fcsPass
    error('generate_uwb_tx_from_decode:FcsFailed', ...
...
if isempty(psduBits) || mod(numel(psduBits), 8) ~= 0
    error('generate_uwb_tx_from_decode:InvalidPsdu', ...
...
if samplesPerSync ~= round(samplesPerSync)
    error('generate_uwb_tx_from_decode:NonintegerSync', ...
...
if symbolsPerSync ~= round(symbolsPerSync)
    error('generate_uwb_tx_from_decode:NonintegerPulseGrid', ...
...
if isempty(options.sfd_sequence)
    ...
    if any(~ismember(selectedSfdSequence, [-1, 0, 1]))
        error('generate_uwb_tx_from_decode:InvalidSfdSequence', ...
    ...
    if numel(customSfdSymbols) ~= standardSfdLength
        error('generate_uwb_tx_from_decode:SfdLengthMismatch', ...
...
if ~(ischar(options.phy_mode) || ...
    ...
    error('generate_uwb_tx_from_decode:InvalidOption', ...
...
if ~isstruct(payload)
    error('generate_uwb_tx_from_decode:InvalidInput', ...
