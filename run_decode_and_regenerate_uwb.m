...
%% -------------------- Read the preprocessed capture --------------------
% Reuse the exact same 998.4 MHz complex-baseband vector for decoding and
% regenerated-waveform comparison.
fprintf('Reading preprocessed QM35 capture: %s\n', options.file_name);
raw = uwbdecoder.readIqRaw(options.file_name, options.sample_offset, ...
    options.sample_num, options.ant_num);
rx_preprocessed = uwbdecoder.selectIqChannel(raw, options.channel_index);
interference = struct('enabled', false, 'frequency_hz', NaN, ...
    'coefficient', complex(0), 'suppression_db', NaN, ...
    'source', 'preprocessed input');

%% -------------------- Decode the preprocessed signal --------------------
fprintf('Decoding the preprocessed QM35 signal...\n');
result = decode_uwb(options, rx_preprocessed, interference);
...
comparison_png = fullfile(output_dir, ...
    'preprocessed_vs_regenerated.png');
...
comparison = compare_uwb_original_and_generated( ...
    options, tx, comparison_png, show_comparison_figure, ...
    rx_preprocessed, interference, tx_after_cir);
cancellation = cancel_uwb_with_regenerated( ...
    options, tx_after_cir, rx_preprocessed, ...
...
fprintf('Preprocessed-grid frequency offset: %.3f MHz\n', ...
