...
if cfg.remove_synchronous_tone
...
if ~isfield(cfg, 'remove_synchronous_tone') || isempty(cfg.remove_synchronous_tone)
    cfg.remove_synchronous_tone = false;
end
if ~isfield(cfg, 'require_pll_template') || isempty(cfg.require_pll_template)
    cfg.require_pll_template = false;
end
if ~isfield(cfg, 'require_fcs_pass_for_cancellation') || ...
        isempty(cfg.require_fcs_pass_for_cancellation)
    cfg.require_fcs_pass_for_cancellation = true;
end
cfg.resume = logical(cfg.resume);
cfg.make_plots = logical(cfg.make_plots);
cfg.overwrite = logical(cfg.overwrite);
cfg.remove_synchronous_tone = logical(cfg.remove_synchronous_tone);
cfg.require_pll_template = logical(cfg.require_pll_template);
cfg.require_fcs_pass_for_cancellation = ...
    logical(cfg.require_fcs_pass_for_cancellation);
...
function stage = runDecodeStage(toolingDir, inputFile, profile, ...
        resultDir, algorithm, resume, overwrite, forceRebuild)
if nargin < 8
    forceRebuild = false;
end
matFile = fullfile(resultDir, 'all_frames_cir.mat');
csvFile = fullfile(resultDir, 'frame_summary.csv');
reuseProducts = resume && ~forceRebuild && isfile(matFile) && isfile(csvFile);
if reuseProducts
...
if reuseProducts
    ...
    try
        validateCancellationProvenance(saved, fittingFile, ...
            outputBaseFile, outputFile, profile, algorithm, cfg);
    catch validationError
        if ~overwrite
            rethrow(validationError);
        end
...
if reuseProducts
    ...
    sic_stage_config = struct( ...
        ...
        'remove_synchronous_tone', cfg.remove_synchronous_tone, ...
        'tone_coefficient', toneCoefficient, ...
        'enable_pll_phase_compensation', cfg.require_pll_template, ...
        ...
        'require_fcs_pass', ...
            cfg.require_fcs_pass_for_cancellation); %#ok<NASGU>
...
function validateCancellationProvenance( ...
        saved, fittingFile, outputBaseFile, outputFile, profile, algorithm, cfg)
...
if cfg.remove_synchronous_tone && ~saved.remove_synchronous_tone
...
if isfield(saved, 'require_fcs_pass') && logical(saved.require_fcs_pass) ~= ...
        logical(cfg.require_fcs_pass_for_cancellation)
    error('uwbSicPipeline:CancellationFcsSelectionMismatch', ...
        '%s cancellation used a different FCS selection policy.', profile);
end
...
if cfg.require_pll_template && isstruct(pll) && isfield(pll, 'resolution') && ...
...
if (cfg.require_pll_template && ~validPll) || ...
        ~saved.enable_cir_slow_phase_compensation || ...
...
algorithm = struct();
algorithm.version = [ ...
    'adaptive_fullrate_multipkt_v3_pll10_cir11_', ...
    'cfo2_full_packet_sfo_v1'];
algorithm.detection_algorithm_version = 3;
