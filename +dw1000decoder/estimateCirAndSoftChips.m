...
available_repetitions = min(preamble.detected_repetitions, ...
...
if isempty(params.cir_skip_initial_repetitions)
    ...
    repetition_count = min(params.cir_repetitions, available_repetitions);
    first_repetition = max(0, available_repetitions-repetition_count);
else
    ...
    first_repetition = params.cir_skip_initial_repetitions;
    repetition_count = min(params.cir_repetitions, ...
        available_repetitions-first_repetition);
    if repetition_count < 1
        error(['No detected preamble repetitions remain after skipping ', ...
            'the first %d repetitions.'], first_repetition);
    end
end
...
cir = struct('values', values, 'delay_ns', delay_ns, ...
    ...
    'first_repetition', first_repetition+1, ...
    'last_repetition', last_repetition+1, ...
    'skipped_initial_repetitions', first_repetition, ...
...
if isfield(params, 'verbose') && params.verbose
    fprintf(['Estimated %d-sample CIR (pre=%d, post=%d) from ', ...
        ...
        length(values), pre_samples, post_samples, ...
        cir.first_repetition, cir.last_repetition, valid_count, ...
