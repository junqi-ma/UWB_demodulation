...
F:\UWB基带数据\qm35_scheduled_sc16_dump\
  capture.iq      mixed，原生 SC16 @737.28 MS/s，窗首尾相接
  capture.jsonl   100 行，每窗一行

F:\UWB基带数据\qm35_clean_scheduled_sc16_dump\
  capture.iq      无干扰 QM35，同样格式
  capture.jsonl   99 行（1 acq + 2 provisional + 96 scheduled）
```

格式见 [[GNURadio_scheduled_SC16_dump数据解读]]。clean 没有 DW1000，是对照
“seed 是否锁到头里干扰”的负例。C++ 在 clean 上是 **98/99 FCS**（id=24
`payload_failed`，与 live 解调同一包）。

### 对照结果（C++ 为真值）

```text
F:\USRP数据解调\decoded_results\scheduled_sc16_dump\
  scheduled_dump_cpp.csv              mixed C++ 真值
...
F:\USRP数据解调\decoded_results\scheduled_sc16_dump_qm35_clean\
  scheduled_dump_cpp.csv              无干扰 C++ 真值（98/99）
  scheduled_dump_cpp_summary.json
```
