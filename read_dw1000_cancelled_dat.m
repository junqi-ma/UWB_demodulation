...
if ~isfile(originalFileName)
    error('read_dw1000_cancelled_dat:FileNotFound', ...
...
if channelIdx < 1 || channelIdx > antNum
    error('read_dw1000_cancelled_dat:InvalidChannel', ...
...
if mod(fileInfo.bytes, bytesPerTimeSample) ~= 0
    error('read_dw1000_cancelled_dat:InvalidFileSize', ...
...
if mod(originalFileInfo.bytes, bytesPerTimeSample) ~= 0
    error('read_dw1000_cancelled_dat:InvalidOriginalFileSize', ...
...
if originalTotalSamples ~= totalSamples
    error('read_dw1000_cancelled_dat:LengthMismatch', ...
...
if sampleOffset < 0 || sampleOffset ~= fix(sampleOffset) || ...
    ...
    error('read_dw1000_cancelled_dat:InvalidOffset', ...
...
if isinf(sampleNum)
    ...
    if sampleNum <= 0 || sampleNum ~= fix(sampleNum)
        error('read_dw1000_cancelled_dat:InvalidSampleNum', ...
