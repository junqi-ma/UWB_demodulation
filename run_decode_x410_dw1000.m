...
%% Input capture
options = struct();
options.file_name = 'F:\DW1000_1.dat';
...
timing = appendTiming(timing, 3, 'buildDw1000Reference', 'setup', ...
    toc(step_timer));
...
step_timer = tic;
preamble_detected = dw1000decoder.detectRepeatedPreamble( ...
...
if params.enable_frame_crop
    [rx_work, preamble_detected, crop_info] = dw1000decoder.cropToFrame( ...
...
if params.show_plots
    step_timer = tic;
    dw1000decoder.plotPreambleDetection( ...
...
step_timer = tic;
[rx_corrected, preamble_cfo] = dw1000decoder.compensateCarrierOffset( ...
    rx_work, preamble_detected, reference, params);
...
step_timer = tic;
preamble_refined = dw1000decoder.refineTimingWithNsSfd( ...
